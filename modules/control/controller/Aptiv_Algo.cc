...
Aptiv_AlgoController::Aptiv_AlgoController()
    : name_(ControlConf_ControllerType_Name(ControlConf::AptivAlgo_CONTROLLER)) {
...
void Aptiv_AlgoController::CloseLogFile() {
  if (FLAGS_enable_csv_debug) {
    if (speed_log_file_ != nullptr) {
...
void Aptiv_AlgoController::Stop() { CloseLogFile(); }

Aptiv_AlgoController::~Aptiv_AlgoController() { CloseLogFile(); }

Status Aptiv_AlgoController::Init(std::shared_ptr<DependencyInjector> injector,
                           const ControlConf *control_conf) {
  control_conf_ = control_conf;
  if (control_conf_ == nullptr) {
    controller_initialized_ = false;
    AERROR << "get_AptivAlgo_param() nullptr";
...
Status Aptiv_AlgoController::ComputeControlCommand(
...
Status Aptiv_AlgoController::Reset() {
  speed_pid_controller_.Reset();
  station_pid_controller_.Reset();
  return Status::OK();
}

std::string Aptiv_AlgoController::Name() const { return name_; }

void Aptiv_AlgoController::ComputeLongitudinalErrors(
...
void Aptiv_AlgoController::SetDigitalFilter(double ts, double cutoff_freq,
...
void Aptiv_AlgoController::GetPathRemain(SimpleLongitudinalDebug *debug) {
  int stop_index = 0;
